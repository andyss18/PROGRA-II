...
public class Empleado extends Persona {
    ...
    public DefaultTableModel mostrar() {
        ...
        try {
            ...
            String query = "SELECT e.id_empleado as id,e.codigo,e.nombres,e.apellidos,e.direccion,e.telefono,e.fecha_nacimiento as Nacimiento,p.puesto,p.id_puesto FROM bd_proof.empleados as e,bd_proof.puestos as p where e.id_puesto = p.id_puesto;";
            ...
            while (consulta.next()) {
                ...
                datos[6] = consulta.getString("Nacimiento");
    ...
    public int modificar(){
        ...
        try{
            PreparedStatement parametro;
            String query="UPDATE `bd_proof`.`empleados` SET `codigo` = ?, `nombres` = ?, "
                    + "`apellidos` = ?, `direccion` = ?, `telefono` = ?, "
                    + "`fecha_nacimiento` = ?, `id_puesto` = ? WHERE (`id_empleado` = ?);";
